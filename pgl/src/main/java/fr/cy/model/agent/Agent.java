...
public class Agent implements StressInducing, Serializable {
    ...
    /**
     * Performs the current action of the agent, if any, and returns the amount of
     * time consumed
     */
    ...
    public double getStressInducingImpact() {
        return Math.max(-0.5,
                Math.min(behavioralState.getStressLevel() * 0.1 + getEmotionalState().getStressInducedToOthers(), 1.0)); // FIXME:
                                                                                                                         // temporary
    }
